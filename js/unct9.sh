...
check_root(){
	[[ $EUID != 0 ]] && echo -e "${Error} 本机权限不足。" && exit 1
}
check_sys(){
...
SSR_installation_status(){
	[[ ! -e ${config_user_file} ]] && echo -e "${Error} 配置文件不存在，请重新安装 !" && exit 1
	[[ ! -e ${ssr_folder} ]] && echo -e "${Error} 程序安装目录不存在 !" && exit 1
}
...
View_User(){
	...
	if [[ -z "${now_mode}" ]]; then
		...
		[[ ${user_total} = "0" ]] && echo -e "${Error} 程序未设置多端口，请检查 !" && exit 1
		clear && echo "===================================================" && echo
		echo -e " 配置信息：" && echo
		echo -e " I  P\t    : ${Green_font_prefix}${ip}${Font_color_suffix}"
		echo -e " 加密\t    : ${Green_font_prefix}${method}${Font_color_suffix}"
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/service/ssr_centos -O /etc/rc.d/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/rc.d/init.d/ssr
		chkconfig --add ssr
		chkconfig ssr on
	else
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/service/ssr_debian -O /etc/rc.d/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/rc.d/init.d/ssr
...
Uninstall_SSR(){
	...
	if [[ ${unyn} == [Yy] ]]; then
		...
		rm -rf ${ssr_folder} && rm -rf ${config_folder} && rm -rf /etc/rc.d/init.d/ssr
...
Stop_SSR(){
	...
	/etc/rc.d/init.d/ssr stop
}
Restart_SSR(){
	SSR_installation_status
	check_pid
	[[ ! -z ${PID} ]] && /etc/rc.d/init.d/ssr stop
	/etc/rc.d/init.d/ssr start
	check_pid
	[[ ! -z ${PID} ]] && View_User
}
...
Update_Shell(){
	...
	if [[ -e "/etc/rc.d/init.d/ssr" ]]; then
		rm -rf /etc/rc.d/init.d/ssr
		Service_SSR
	fi
